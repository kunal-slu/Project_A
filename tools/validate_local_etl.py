#!/usr/bin/env python3
"""
Local ETL Pipeline Validation Script

Validates that the Bronze → Silver → Gold pipeline works correctly by:
1. Reading Silver and Gold output tables
...
def read_table(
    ...
    *,
    ...
    For non-local: delta first, then parquet fallback
    ...
    # Local: parquet only (no delta attempts)
    if is_local:
        try:
            ...
            if os.path.exists(path) and schema is not None:
                logger.warning(
                    f"  ⚠ {table_name} path exists but appears empty; returning empty DF with explicit schema"
                )
            ...
            # Return empty DataFrame with provided schema or minimal schema
    ...
    # Non-local: delta first, then parquet fallback
    try:
        ...
        logger.warning(
            f"  ⚠ {table_name} not found as delta from {path} ({delta_err}); falling back to parquet"
        )
        try:
            df = spark.read.parquet(path)
            count = df.count()
            logger.info(f"  ✓ {table_name}: {count:,} rows (parquet)")
            return df
        except AnalysisException as pq_err:
            if os.path.exists(path) and schema is not None:
                logger.warning(
                    f"  ⚠ {table_name} path exists but appears empty; returning empty DF with explicit schema"
                )
...
def validate_silver_layer(
    ...
    for table_key, (table_name, required, schema) in silver_tables.items():
        ...
        if table_key == "fx_rates_silver" and results[table_key].get("row_count", 0) == 0:
            if schema is not None:
                # Check if schema was inferred (indicates table exists with schema)
    ...
    for table_key in optional_tables:
        if table_key in dfs:
            ...
            if count > 0:
                ...
                # Check if schema is present (indicates table was created but is empty)
                if results[table_key].get("schema") is not None:
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Validate local ETL pipeline")
    ...
    # Build Spark session
    spark = build_spark(app_name="validate_local_etl", config=config)
    
    try:
        print("="*80)
        print("🚀 LOCAL ETL PIPELINE VALIDATION")
        print("="*80)
        print(f"Environment: {config.get('environment', 'local')}")
