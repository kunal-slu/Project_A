...
salesforce_extraction_task = EmrServerlessStartJobOperator(
    ...
    job_driver={
        'sparkSubmit': {
            'entryPoint': 's3://{}/jobs/salesforce_to_bronze.py'.format(S3_BUCKET),
            'entryPointArguments': [
...
data_quality_check = PythonOperator(
    task_id='salesforce_data_quality_check',
    python_callable=check_salesforce_data_quality,
    dag=dag
)
...
# Task dependencies
salesforce_extraction_task >> wait_for_extraction >> data_quality_check >> register_glue_tables >> emit_lineage_metrics