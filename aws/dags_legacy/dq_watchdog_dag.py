...
def check_data_quality(**context):
    ...
    dq_results = {"bronze": {}, "silver": {}, "issues": []}
    ...
    for table in ["customers", "orders", "products", "payments"]:
        try:
            ...
            latest_partition = s3_client.list_objects_v2(
                Bucket=bucket_name, Prefix=f"bronze/{table}/"
            )

            if "Contents" in latest_partition:
                latest_file = max(latest_partition["Contents"], key=lambda x: x["LastModified"])
                hours_old = (
                    datetime.utcnow() - latest_file["LastModified"].replace(tzinfo=None)
                ).total_seconds() / 3600

                if hours_old > DQ_THRESHOLDS["freshness_hours"]:
                    dq_results["bronze"][table] = {"status": "STALE", "hours_old": hours_old}
                    dq_results["issues"].append(
                        f"Bronze {table}: Stale data ({hours_old:.1f} hours old)"
                    )
                else:
    ...
    for table in ["customers_clean", "orders_clean", "products_clean"]:
        try:
            latest_partition = s3_client.list_objects_v2(
                Bucket=bucket_name, Prefix=f"silver/{table}/"
            )
            ...
            if "Contents" in latest_partition:
                ...
                if hours_old > DQ_THRESHOLDS["freshness_hours"]:
                    dq_results["silver"][table] = {"status": "STALE", "hours_old": hours_old}
                    dq_results["issues"].append(
                        f"Silver {table}: Stale data ({hours_old:.1f} hours old)"
                    )
                else:
