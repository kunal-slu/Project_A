...
with DAG(
    ...
    is_production = (
        os.getenv("AIRFLOW_ENV") == "production"
        or Variable.get("PROJECT_A_ENV", default_var="dev") == "prod"
    )
    ...
    if is_production:
        ...
        try:
            ...
            raise ValueError(
                "PROJECT_A_ARTIFACTS_BUCKET Airflow Variable is required in production"
            )
    else:
        # Local dev: use safe defaults
        emr_app_id = Variable.get("PROJECT_A_EMR_APP_ID", default_var="00g0tm6kccmdcf09")
        emr_exec_role_arn = Variable.get(
            "PROJECT_A_EXEC_ROLE_ARN",
            default_var="arn:aws:iam::424570854632:role/project-a-dev-emr-exec",
        )
        artifacts_bucket = Variable.get(
            "PROJECT_A_ARTIFACTS_BUCKET", default_var="my-etl-artifacts-demo-424570854632"
        )

    # Environment and config (can have defaults)
    env = Variable.get("PROJECT_A_ENV", default_var="dev")
    config_uri = Variable.get(
        "PROJECT_A_CONFIG_URI", default_var=f"s3://{artifacts_bucket}/config/dev.yaml"
    )
        ...
        snowflake_bronze = EmrServerlessStartJobRunOperator(
            ...
            configuration_overrides={
                "monitoringConfiguration": {
                    "s3MonitoringConfiguration": {"logUri": f"s3://{artifacts_bucket}/emr-logs/"}
                }
            },
    ...
    # ============================================================================
    (
        [snowflake_bronze, crm_bronze, redshift_bronze, fx_bronze, kafka_bronze]
        >> bronze_complete
        >> bronze_to_silver
        >> dq_silver
        >> silver_to_gold
