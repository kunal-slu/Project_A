...
def send_dq_alert(results: dict, config: dict):
    ...
    if slack_webhook:
        try:
            ...
            message = {
                "text": f"❌ Data Quality Check Failed",
                "blocks": [
                    {
                        "type": "section",
                        "text": {
...
if __name__ == "__main__":
    ...
    parser.add_argument("--critical-only", action="store_true", help="Only run critical expectations")
    parser.add_argument("--output", default="data/metrics/dq_results", help="Output path for results")
    parser.add_argument("--fail-on-error", action="store_true", default=True, help="Exit with error code on failure")
