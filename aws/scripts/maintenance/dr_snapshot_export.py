...
class DRSnapshotExporter:
    ...
    def get_latest_partitions(self, layer: str, table: str, days_back: int = 7) -> list[str]:
        ...
        try:
            ...
            pages = paginator.paginate(Bucket=self.primary_bucket, Prefix=prefix, Delimiter="/")
    ...
    def copy_partition(self, source_key: str, dest_key: str) -> bool:
        ...
        try:
            ...
            self.dr_s3.copy_object(CopySource=copy_source, Bucket=self.dr_bucket, Key=dest_key)
    ...
    def export_table(self, layer: str, table: str, days_back: int = 7) -> dict[str, Any]:
        ...
        logger.info(
            f"Export completed: {stats['partitions_copied']}/{stats['partitions_found']} partitions copied"
        )
    ...
    def export_all_tables(self, layers: list[str] = None, days_back: int = 7) -> dict[str, Any]:
        ...
        tables_config = self.config.get("dr", {}).get(
            "tables",
            {
        ...
        logger.info("DR snapshot export completed:")
...
def main():
    ...
    parser.add_argument("--layers", nargs="+", default=["silver", "gold"], help="Layers to export")
    parser.add_argument(
        "--days-back", type=int, default=7, help="Days of data to export (default: 7)"
    )
