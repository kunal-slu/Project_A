...
class LineageEmitter:
    ...
    def create_run_metadata(
        self, job_name: str, input_tables: list[str], output_tables: list[str], start_time: datetime
    ) -> RunMetadata:
    ...
    def emit_lineage_event(
        self, run_metadata: RunMetadata, transformations: list[str], data_quality_score: float
    ) -> LineageEvent:
    ...
    def log_table_metrics(
        self, run_metadata: RunMetadata, table_name: str, row_count: int, s3_path: str
    ) -> None:
    ...
    def finalize_run_metadata(
        self, run_metadata: RunMetadata, status: str, end_time: datetime
    ) -> None:
        """Finalize run metadata with completion information."""

        run_metadata.end_time = end_time.isoformat()
        run_metadata.status = status
        run_metadata.processing_time_seconds = (
            end_time - datetime.fromisoformat(run_metadata.start_time)
        ).total_seconds()
        ...
        logger.info(
            f"Run {run_metadata.run_id} completed: {status}, "
