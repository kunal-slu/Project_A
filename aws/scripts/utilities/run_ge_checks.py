...
def run_yaml_policy(df, policy: dict, key_cols=None):
            ...
            if col in df.columns:
                ...
                if total_count == unique_count:
                    results["checks"].append(
                        {"check": f"uniqueness_{col}", "passed": True, "severity": "critical"}
                    )
    ...
    for col in policy.get("not_null", []):
        if col in df.columns:
            ...
            if null_count == 0:
                results["checks"].append(
                    {"check": f"not_null_{col}", "passed": True, "severity": "critical"}
                )
                results["summary"]["passed"] += 1
            else:
                results["checks"].append(
    ...
    for col, range_config in policy.get("range", {}).items():
        if col in df.columns:
            ...
            if invalid_count == 0:
                results["checks"].append(
                    {"check": f"range_{col}", "passed": True, "severity": "warning"}
                )
                results["summary"]["passed"] += 1
            else:
                results["checks"].append(
...
def run_expectation_checks(
    spark, table_location: str, suite: dict[str, Any], table_name: str
) -> dict[str, Any]:
    """
    ...
    try:
        if expectation_type == "expect_column_to_exist":
            ...
            if total_count == unique_count:
                ...
                check_result["error"] = (
                    f"Column '{column}' has {total_count - unique_count} duplicate values"
                )
            ...
            if total_count == unique_count:
                ...
                check_result["error"] = (
                    f"Compound columns {columns} have {total_count - unique_count} duplicate values"
                )
            ...
            if invalid_count == 0:
                ...
                check_result["error"] = (
                    f"Column '{column}' has {invalid_count} values outside range [{min_value}, {max_value}]"
                )
...
def write_results_to_s3(results: dict[str, Any], s3_path: str) -> None:
    ...
    try:
        s3_client.put_object(
            Bucket=bucket, Key=full_key, Body=results_json, ContentType="application/json"
        )
