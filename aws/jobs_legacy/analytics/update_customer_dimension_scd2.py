...
from pyspark.sql.functions import (
    col, when, lit, current_timestamp, max as spark_max,
...
def update_customer_dimension_scd2(spark: SparkSession, config: Dict[str, Any]) -> None:
    ...
    try:
        ...
        try:
            existing_customers_df = read_delta(spark, f"{config['lake']['gold']}/dim_customers_scd2")
            logger.info(f"Found existing customer dimension: {existing_customers_df.count()} records")
        except:
        ...
        if existing_customers_df.count() == 0:
            write_delta(new_customers_scd2, f"{config['lake']['gold']}/dim_customers_scd2", mode="overwrite")
        ...
        key_fields = ["first_name", "last_name", "email", "address", "city", "state", "country", "phone", "age"]
        ...
        changed_records = new_customers_scd2.alias("new").join(
            existing_customers_df.filter(col("is_current") == True).alias("existing"),
            "customer_id",
            "inner"
        ).filter(has_changes).select(
            col("new.*")
        )
        ...
        if changed_records.count() > 0:
            ...
            existing_customers_updated = existing_customers_df.join(
                customers_to_close,
                "customer_id",
                "left"
            ).withColumn(
                ...
                "valid_to",
                when(col("customer_id").isNotNull(), current_timestamp()).otherwise(col("valid_to"))
            ).withColumn(
                "updated_at",
                when(col("customer_id").isNotNull(), current_timestamp()).otherwise(col("updated_at"))
            ).drop("customer_id")
            
            # Prepare new records for changed customers
            new_records_scd2 = changed_records.withColumn(
                "change_reason",
                lit("data_update")
            )
            ...
            new_customers_only = new_customers_scd2.join(
                existing_customers_df.select("customer_id").distinct(),
                "customer_id",
                "left_anti"
            )
            ...
            write_delta(all_records, f"{config['lake']['gold']}/dim_customers_scd2", mode="overwrite")
            
            logger.info(f"Customer dimension updated: {all_records.count()} total records")
            
            # Log SCD2 statistics
            current_records = all_records.filter(col("is_current") == True)
            historical_records = all_records.filter(col("is_current") == False)
