...
from pyspark.sql.functions import (
    col, when, lit, current_timestamp, sum as spark_sum, count,
    avg, max as spark_max, min as spark_min, first, last
)
...
def build_marketing_attribution(spark: SparkSession, config: Dict[str, Any]) -> None:
    ...
    try:
        ...
        ], ["customer_id", "acquisition_channel", "campaign_name", "acquisition_cost", "first_touch_date"])
        ...
        attribution_df = attribution_df.withColumn(
            ...
            col("total_revenue") - col("acquisition_cost")
        )
        
        attribution_df = attribution_df.withColumn(
            "roi",
            when(col("acquisition_cost") > 0, 
        ...
        attribution_df = attribution_df.withColumn(
            "payback_period_days",
            when(col("acquisition_cost") > 0, 
                 col("acquisition_cost") / (col("total_revenue") / 365))
            .otherwise(0)
        )
