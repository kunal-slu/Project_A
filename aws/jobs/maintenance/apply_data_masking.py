...
class DataMaskingEngine:
    ...
    def __init__(self, spark: SparkSession, config: dict[str, Any]):
        ...
        self.pii_columns = (
            config.get("security", {}).get("data_classification", {}).get("pii_columns", [])
        )
        self.sensitive_columns = (
            config.get("security", {}).get("data_classification", {}).get("sensitive_columns", [])
        )
    ...
    def apply_masking_strategies(self, df: DataFrame, table_name: str) -> DataFrame:
        ...
        df = (
            df.withColumn("data_masking_applied", lit(True))
    ...
    def _get_masking_strategy(self, column: str, classification: str) -> str:
        ...
        elif classification == "sensitive" and any(
            fin_term in column_lower for fin_term in ["salary", "revenue", "profit"]
        ):
        ...
        if strategy == "email_hash":
            ...
            df = df.withColumn(
                ...
                when(
                    ...
                    concat(lit("user_"), hash(col(column)).cast(StringType()), lit("@masked.com")),
            ...
            df = df.withColumn(
                column,
                when(col(column).isNotNull(), regexp_replace(col(column), r"\d", "X")).otherwise(
                    col(column)
                ),
            )
            ...
            df = df.withColumn(
                column,
                when(col(column).isNotNull(), (col(column) / 1000).cast("int") * 1000).otherwise(
                    col(column)
                ),
            )
            ...
            df = df.withColumn(
                column,
                when(col(column).isNotNull(), hash(col(column)).cast(StringType())).otherwise(
                    col(column)
                ),
            )
...
def mask_silver_to_gold_table(
    spark: SparkSession, config: dict[str, Any], source_table: str, target_table: str
) -> None:
