...
def upload_to_s3(df, logical_name: str, config: dict[str, Any], ingest_date: str) -> None:
    ...
    try:
        ...
        s3_key = (
            f"bronze/crm/salesforce/{logical_name}/ingest_date={ingest_date}/{logical_name}.parquet"
        )
    ...
    try:
        ...
        if table_name == "contacts":
            ...
            if invalid_email_count > 0:
                logger.warning(
                    f"Data quality warning: {invalid_email_count} contacts have invalid email format"
                )
            ...
            if null_stage_count > 0:
                logger.error(
                    f"Data quality check failed: {null_stage_count} opportunities have null stage_name"
                )
                return False
            ...
            if negative_amount_count > 0:
                logger.warning(
                    f"Data quality warning: {negative_amount_count} opportunities have non-positive amount"
                )
            ...
            if null_name_count > 0:
                logger.error(
                    f"Data quality check failed: {null_name_count} accounts have null account_name"
                )
                return False
...
def main():
    ...
    try:
        ...
        opportunities_df = extract_salesforce_opportunities(
            spark, config, "salesforce_opportunities"
        )
