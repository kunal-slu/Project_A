...
def main():
    ...
    try:
        ...
        load_kafka_bronze_data(spark, config)

        # Build silver tables
        df_customers_silver = build_customers_silver(
            df_customers, df_accounts, df_contacts, df_opps, df_behavior
        )
        ...
        write_table(
            df_customers_silver,
            f"{silver_root}/{tables_config.get('customers', 'customers_silver')}",
            config,
        )
        write_table(
            df_orders_silver,
            f"{silver_root}/{tables_config.get('orders', 'orders_silver')}",
            config,
        )
        write_table(
            df_products_silver,
            f"{silver_root}/{tables_config.get('products', 'products_silver')}",
            config,
        )
        write_table(
            df_behavior_silver,
            f"{silver_root}/{tables_config.get('behavior', 'customer_behavior_silver')}",
            config,
        )
