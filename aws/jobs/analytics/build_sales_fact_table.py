...
from pyspark.sql.functions import (
    ...
    year, month, quarter, sum as spark_sum, count, avg, max as spark_max, min as spark_min,
    first, last, collect_list, collect_set, array_distinct, size,
    row_number, rank, dense_rank, lag, lead, window,
    broadcast, coalesce, regexp_replace, split, explode,
    percentile_approx, stddev, variance, skewness, kurtosis,
    grouping_id, cube, rollup, pivot, unpivot
)
...
def build_sales_fact_table(spark: SparkSession, config: Dict[str, Any]) -> None:
    ...
    try:
        ...
        products_df = read_delta(spark, f"{config['lake']['silver']}/products_clean")
        ...
        sales_fact = sales_fact.withColumn("order_day_of_week", date_format(col("order_date"), "EEEE"))
