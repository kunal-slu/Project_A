...
def test_event_name_is_lowercase(silver_behavior_df):
    ...
    non_lowercase = silver_behavior_df.filter(col("event_name") != col("event_name").lower())
...
def test_session_id_pattern(silver_behavior_df):
    ...
    invalid_pattern = silver_behavior_df.filter(
        col("session_id").isNotNull() & ~col("session_id").rlike(r"^SESS-\d+$")
    )
...
def test_event_ts_is_timestamp(silver_behavior_df):
    ...
    # Check data type
    event_ts_field = next(
        (f for f in silver_behavior_df.schema.fields if f.name == "event_ts"), None
    )
...
def test_exactly_expected_columns(silver_behavior_df):
    ...
    expected_min_columns = (
        7  # event_id, customer_id, event_name, event_ts, session_id, page_url, referrer
    )
...
def test_types_match_contract(silver_behavior_df):
    ...
    for col_name, expected_type in type_checks.items():
        ...
        field = next((f for f in silver_behavior_df.schema.fields if f.name == col_name), None)
...
def test_row_count_threshold(silver_behavior_df):
    ...
    assert actual_rows >= min_rows, f"Expected at least {min_rows} rows, got {actual_rows}"
