...
def sample_orders_data(spark):
    """Sample orders data."""
    data = [
        ("O1", "C1", "P1", "2025-01-01", Decimal("100.00"), 2, "USD", "COMPLETED"),
        ("O2", "C2", "P2", "2025-01-02", Decimal("200.00"), 1, "USD", "COMPLETED"),
    ]
    schema = StructType(
        [
...
def test_build_customer_360_success(
    ...
    sample_contacts_data.write.format("delta").mode("overwrite").save(
        str(silver_path / "crm" / "contacts")
    )
    sample_accounts_data.write.format("delta").mode("overwrite").save(
        str(silver_path / "crm" / "accounts")
    )
    sample_orders_data.write.format("delta").mode("overwrite").save(
        str(silver_path / "snowflake" / "orders")
    )
...
@patch("jobs.silver_build_customer_360.emit_metrics")
def test_build_customer_360_handles_missing_data(mock_emit_metrics, spark, mock_config, tmp_path):
