...
class TestSCD2Validation:
    ...
    def sample_scd2_data(self, spark):
        ...
        data = [
            ("C001", "123 Main St", date(2020, 1, 1), date(2023, 6, 15), False),
            ("C001", "456 Oak Ave", date(2023, 6, 15), None, True),
            ("C002", "789 Pine St", date(2021, 3, 1), None, True),
            ("C003", "321 Elm St", date(2022, 1, 1), date(2023, 12, 1), False),
            ("C003", "654 Maple Dr", date(2023, 12, 1), None, True),
        ]
    ...
    def test_scd2_required_columns(self, sample_scd2_data):
        ...
        assert required_cols.issubset(actual_cols), (
            f"Missing required columns: {required_cols - actual_cols}"
        )
    ...
    def test_scd2_single_current_record_per_key(self, sample_scd2_data):
        ...
        violations = (
            sample_scd2_data.filter(F.col("is_current"))
            .groupBy("customer_id")
            .count()
            .filter("count != 1")
        )
    ...
    def test_scd2_no_date_overlaps(self, sample_scd2_data):
        ...
        overlaps = sample_scd2_data.withColumn(
            "next_from", F.lead("effective_from").over(w)
    ...
    def test_scd2_effective_from_before_effective_to(self, sample_scd2_data):
        ...
        violations = sample_scd2_data.filter(
            (~F.col("is_current")) & (F.col("effective_from") >= F.col("effective_to"))
        )
    ...
    def test_scd2_no_duplicate_records(self, sample_scd2_data):
        ...
        duplicates = (
            sample_scd2_data.groupBy("customer_id", "effective_from").count().filter("count > 1")
        )

        assert duplicates.count() == 0, (
            "Found duplicate records for same business key and effective_from"
        )
    ...
    def test_scd2_continuous_date_ranges(self, sample_scd2_data):
        ...
        gaps = sample_scd2_data.withColumn("prev_to", F.lag("effective_to").over(w)).filter(
            "prev_to is not null and effective_from > prev_to"
        )
...
def validate_scd2_table(
    spark: SparkSession, table_path: str, business_key: str = "customer_id"
) -> dict:
    """
    ...
    try:
        ...
        results = {
            ...
            "current_records": df.filter(F.col("is_current")).count(),
            "historical_records": df.filter(~F.col("is_current")).count(),
        ...
        violations = (
            df.filter(F.col("is_current")).groupBy(business_key).count().filter("count != 1")
        )
        ...
        overlaps = df.withColumn("next_from", F.lead("effective_from").over(w)).filter(
            "next_from is not null and next_from < effective_to"
        )
        if overlaps.count() > 0:
            results["validation_passed"] = False
