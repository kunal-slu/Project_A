...
def test_orders_silver_schema_defined():
    ...
    assert "amount_usd" not in field_names, \
        "Schema should use 'total_amount', not 'amount_usd'"
...
def test_bronze_to_silver_transformation_mock(spark):
    ...
    bronze_data = [
        ("ORD001", "CUST001", "PROD001", date(2024, 1, 1), 100.50, 2, "completed", 
    ...
    assert "total_amount" in silver_df.columns, \
        "Silver layer must have 'total_amount' column"
    assert "amount_usd" not in silver_df.columns, \
        "Silver layer should not have 'amount_usd' column"
...
def test_schema_apply_enforcement(spark):
    ...
    data = [
        ("ORD001", "CUST001", "PROD001", "2024-01-01", "100.50", "2", "completed", None)
    ]
