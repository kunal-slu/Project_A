...
class TestMetricsCollection:
    ...
    def test_track_job_execution_success(self):
        ...
        assert (
            'etl_job_executions_total{environment="test",job_name="test_job",status="success"}'
            in metrics
        )
    ...
    def test_track_job_execution_failure(self):
        ...
        metrics = get_metrics_text().decode("utf-8")
        assert (
            'etl_job_executions_total{environment="test",job_name="test_job_fail",status="failure"}'
            in metrics
        )
    ...
    def test_record_dq_check_passed(self):
        ...
        metrics = get_metrics_text().decode("utf-8")
        assert (
            'etl_dq_checks_total{check_type="null_check",status="passed",table="test_table"}'
            in metrics
        )
    ...
    def test_record_dq_check_failed_with_violations(self):
        ...
        record_dq_check(
            "test_table", "range_check", passed=False, violations=10, severity="critical"
        )

        metrics = get_metrics_text().decode("utf-8")
        assert (
            'etl_dq_checks_total{check_type="range_check",status="failed",table="test_table"}'
            in metrics
        )
