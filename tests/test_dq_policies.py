...
def test_dq_yaml_schema_validation():
    ...
    required_schema_keys = ["table", "rules"]

    rule_required_keys = ["name", "type"]
        ...
        try:
            ...
            for rule in data["rules"]:
                ...
                assert rule["type"] in valid_rule_types, (
                    f"{yaml_file} invalid rule type: {rule['type']}"
                )
        ...
        try:
            ...
            for rule in data.get("rules", []):
                ...
                if rule_type == "not_null":
                    ...
                    assert "source_column" in rule, (
                        f"{yaml_file} referential_integrity missing source_column"
                    )
                    assert "target_table" in rule, (
                        f"{yaml_file} referential_integrity missing target_table"
                    )
                    assert "target_column" in rule, (
                        f"{yaml_file} referential_integrity missing target_column"
                    )
                ...
                if "severity" in rule:
                    ...
                    assert rule["severity"] in valid_severities, (
                        f"{yaml_file} invalid severity: {rule['severity']}"
                    )
    ...
    for yaml_file in yaml_files:
        try:
            ...
            if table_name:
                ...
                assert "_" in table_name or table_name.islower(), (
                    f"{yaml_file} table name should be lowercase with underscores"
                )
