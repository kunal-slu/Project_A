...
def sample_gold_data(spark):
    ...
    data = [
        ("C1", "John", "Doe", "john@example.com", "ACC1", 500.0, 5),
        ("C2", "Jane", "Smith", "jane@example.com", "ACC2", 1000.0, 10),
    ]
    schema = StructType([
...
def test_load_to_snowflake_success(
    mock_get_creds, mock_emit_metrics, mock_emit_start, mock_emit_complete, mock_emit_fail,
    spark, mock_config, sample_gold_data, tmp_path
):
    ...
    with patch.object(spark.read.format("delta"), "load", return_value=sample_gold_data):
        with patch.object(sample_gold_data.write.format("snowflake"), "save") as mock_write:
            with patch.object(spark.read.format("jdbc"), "load") as mock_jdbc:
                ...
                try:
                    result = load_customer_360_to_snowflake(spark, mock_config)
                except Exception:
...
@patch('jobs.publish_gold_to_snowflake.get_snowflake_credentials')
def test_load_to_snowflake_missing_data(
    mock_get_creds, spark, mock_config, tmp_path
):
