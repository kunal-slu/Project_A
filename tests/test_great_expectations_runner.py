...
@patch("project_a.dq.great_expectations_runner.DataContext")
def test_ge_runner_run_checkpoint_no_context(_mock_data_context):
...
@patch("project_a.dq.great_expectations_runner.DataContext")
def test_ge_runner_run_checkpoint_success(_mock_data_context):
    """Test successful checkpoint run."""
    runner = GreatExpectationsRunner()
    mock_context = Mock()
    mock_checkpoint = Mock()
    mock_context.get_checkpoint.return_value = mock_checkpoint
    mock_checkpoint.run.return_value = {"success": True, "run_results": {}}
...
@patch("project_a.dq.great_expectations_runner.DataContext")
def test_ge_runner_run_checkpoint_failure(_mock_data_context):
    """Test checkpoint run with failure."""
    runner = GreatExpectationsRunner()
    mock_context = Mock()
    mock_checkpoint = Mock()
    mock_context.get_checkpoint.return_value = mock_checkpoint
    mock_checkpoint.run.return_value = {"success": False, "run_results": {}}
    runner.context = mock_context
...
def test_run_dq_checkpoint_convenience():
    ...
    with patch(
        "project_a.dq.great_expectations_runner.GreatExpectationsRunner"
