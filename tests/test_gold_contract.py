...
def test_bronze_schema_validation(schema_validator):
    ...
    schema = StructType(
        [StructField("id", StringType(), True), StructField("data", StringType(), True)]
    )

    df = schema_validator.spark.createDataFrame([], schema)

    # Bronze validation should always pass
    assert schema_validator.validate_bronze_schema(df, "test_table")
...
def test_silver_schema_compatibility(schema_validator):
    ...
    try:
        assert schema_validator.validate_silver_schema(
            compatible_df, "test_table", existing_schema_path
        )
    finally:
    ...
    try:
        assert not schema_validator.validate_silver_schema(
            incompatible_df, "test_table", existing_schema_path
        )
    finally:
        if Path(existing_schema_path).exists():
            Path(existing_schema_path).unlink()
...
def test_gold_schema_contract(schema_validator, fact_orders_contract):
    ...
    try:
        ...
        assert not schema_validator.validate_gold_schema(matching_df, "fact_orders", contract_path)
        ...
        assert not schema_validator.validate_gold_schema(
            non_matching_df, "fact_orders", contract_path
        )
...
def test_schema_contract_file_not_found(schema_validator):
    ...
    assert not schema_validator.validate_gold_schema(df, "test_table", "nonexistent.json")
