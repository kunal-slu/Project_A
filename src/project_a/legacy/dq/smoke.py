...
def assert_non_null(df: DataFrame, col: str, threshold: float = 0.1) -> None:
    ...
    if null_percentage > threshold:
        raise ValueError(f"Column '{col}' has {null_percentage:.2%} nulls (threshold: {threshold:.2%})")
...
def assert_no_duplicates(df: DataFrame, cols: List[str], threshold: float = 0.05) -> None:
    ...
    if duplicate_percentage > threshold:
        raise ValueError(f"Duplicate records: {duplicate_percentage:.2%} (threshold: {threshold:.2%})")
...
def assert_positive_values(df: DataFrame, col: str) -> None:
    ...
    negative_count = df.selectExpr(f"sum(case when {col} < 0 then 1 else 0 end) as n").collect()[0]["n"]
...
def assert_valid_dates(df: DataFrame, col: str) -> None:
    ...
    invalid_count = df.selectExpr(f"sum(case when {col} is null or {col} < '1900-01-01' then 1 else 0 end) as n").collect()[0]["n"]
