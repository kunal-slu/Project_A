...
def write_to_dlq(
    ...
    rejected_with_metadata = (
        rejected_df.withColumn("_dlq_error_type", lit(error_type))
        .withColumn("_dlq_error_reason", lit(error_reason))
        .withColumn("_dlq_timestamp", current_timestamp())
        .withColumn("_dlq_processing_date", lit(processing_date))
    )

    # Write rejected records
    logger.warning(f"Writing {rejected_df.count()} rejected records to DLQ: {dlq_path}")
    rejected_with_metadata.write.format("delta").mode("overwrite").option(
        "mergeSchema", "true"
    ).partitionBy("_dlq_processing_date").save(f"{dlq_path}rejected_rows")
    ...
    success_df = spark.createDataFrame(
        [(processing_date, datetime.utcnow().isoformat())], ["processing_date", "completed_at"]
    )
    success_df.write.format("delta").mode("overwrite").save(f"{dlq_path}_SUCCESS")
...
def quarantine_data(
    ...
    quarantine_base = config.get("data_lake", {}).get(
        "quarantine_prefix", "s3://bucket/_quarantine"
    )
    ...
    df.withColumn("_quarantine_reason", lit(reason)).withColumn(
        "_quarantine_timestamp", current_timestamp()
    ).write.format("delta").mode("overwrite").partitionBy("_quarantine_timestamp").save(
        quarantine_path
    )
