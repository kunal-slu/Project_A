...
from project_a.production_pipeline import ProductionETLPipeline
...
def run_validate(config: dict, env: str):
    ...
    try:
            ...
            if os.path.exists(layer_path):
                for table_dir in os.listdir(layer_path):
                    ...
                    if os.path.isdir(table_path):
                        ...
                        logger.info(f"📊 {layer}.{table_dir}: {len(parquet_files)} parquet files, {len(log_files)} versions")
        ...
        logger.info(f"✅ Validation completed: {total_tables} tables, {total_files} files, {total_versions} versions")
        return True
...
def main():
    ...
    parser.add_argument("--cmd", choices=["ingest", "transform", "validate", "pipeline", "full"], required=True,
                       help="Command to run")
