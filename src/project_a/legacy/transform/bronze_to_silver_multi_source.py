...
def bronze_to_silver_multi_source(
    spark: SparkSession, config: dict[str, Any], run_date: str = None
) -> dict[str, DataFrame]:
    """
    ...
    silver_customers = customers_bronze.withColumn(
        "effective_ts", F.current_timestamp()
    ...
    if "_ingest_ts" in silver_customers.columns:
        ...
        silver_customers = (
            silver_customers.withColumn("rn", F.row_number().over(window))
            .filter(F.col("rn") == 1)
            .drop("rn")
        )
    else:
    ...
    silver_orders = orders_bronze.withColumn(
        "order_date", F.to_date(F.coalesce(F.col("order_date"), F.col("event_ts")))
    ...
    if "_ingest_ts" in silver_orders.columns:
        ...
        silver_orders = (
            silver_orders.withColumn("rn", F.row_number().over(window))
            .filter(F.col("rn") == 1)
            .drop("rn")
        )
    else:
    ...
    silver_customer_activity = (
        behavior_bronze.withColumn(
            "event_date", F.to_date(F.coalesce(F.col("event_ts"), F.col("timestamp")))
        )
    ...
    if "event_id" in silver_customer_activity.columns:
        if "_ingest_ts" in silver_customer_activity.columns:
            ...
            silver_customer_activity = (
                silver_customer_activity.withColumn("rn", F.row_number().over(window))
                .filter(F.col("rn") == 1)
                .drop("rn")
            )
        else:
    ...
    write_delta(
        silver_customers, f"{silver_root}/customers", mode="overwrite", partitionBy=["country"]
    )

    write_delta(
        silver_orders, f"{silver_root}/orders", mode="overwrite", partitionBy=["order_date"]
    )
    ...
    emit_rowcount(
        "silver_orders_total", silver_orders.count(), {"layer": "silver", "table": "orders"}, config
    )

    emit_rowcount(
        "silver_activity_total",
        silver_customer_activity.count(),
        {"layer": "silver", "table": "customer_activity"},
        config,
    )

    emit_duration(
        "silver_transformation_duration", duration_ms, {"stage": "bronze_to_silver"}, config
    )
...
def join_silver_sources_on_customer_id(
    ...
    orders_enriched = silver_orders.join(
    ...
    silver_behavior.join(
        silver_customers.select("customer_id", "country", "email"), on="customer_id", how="left"
    )
