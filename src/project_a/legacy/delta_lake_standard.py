...
class StandardDeltaLake:
    ...
    def append_data(self, new_data: pd.DataFrame, table_name: str, version: int):
        ...
        parquet_file = os.path.join(self.table_path, f"part-{version:05d}-{table_name}-v{version+1}.parquet")
...
def generate_customers_data():
    ...
    data = {
        ...
        'created_date': [datetime.now() - timedelta(days=np.random.randint(1, 365)) for _ in range(1000)],
...
def generate_orders_data():
    ...
    data = {
        ...
        'order_date': [datetime.now() - timedelta(days=np.random.randint(1, 365)) for _ in range(5000)],
...
def generate_revenue_data():
    ...
    data = {
        'month': [7, 8, 9, 10],
        'total_revenue': [333242.69, 884222.00, 445123.45, 678901.23]
    }
...
def generate_updated_customers_data(original_data, version):
    ...
    new_customers['customer_id'] = range(len(original_data) + version*50 + 1, len(original_data) + (version+1)*50 + 1)
    new_customers['name'] = [f'Customer_v{version}_{i}' for i in range(len(original_data) + version*50 + 1, len(original_data) + (version+1)*50 + 1)]
    new_customers['email'] = [f'customer_v{version}_{i}@example.com' for i in range(len(original_data) + version*50 + 1, len(original_data) + (version+1)*50 + 1)]
