...
def deduplicate_fx_rates(spark: SparkSession, bronze_df) -> DataFrame:
    ...
    # FX rates freshness guard - check if rates are stale (strict in prod)
    latest = bronze_df.agg(F.max("as_of_date")).first()[0]
    if latest is not None:
        is_stale = latest < (date.today() - timedelta(days=2))
        if is_stale and os.environ.get("PROJECT_A_STRICT_FX_FRESHNESS") == "1":
            raise RuntimeError(f"FX rates stale: {latest}")
        if is_stale:
            logger.warning("FX rates appear stale: %s", latest)
...
def validate_fx_rates(df) -> bool:
    ...
    # Empty inputs should fail validation to prevent silent no-op runs.
    if df.rdd.isEmpty():
        logger.error("FX rates validation failed: input DataFrame is empty")
        return False
    ...
    if total_count != unique_count:
        logger.error(
            "Found duplicate (as_of_date, ccy) combinations: %s total, %s unique",
            total_count,
            unique_count,
        )
        return False
...
def main():
    ...
    spark = get_spark_session(app_name="fx-bronze-to-silver", config=get_delta_config())
