...
def build_dim_customer_scd2(
    ...
    if src.isEmpty():
        ...
        return spark.createDataFrame(
            [],
            schema=spark.table("gold.dim_customer").schema
    ...
    src_scd2 = (
        src.withColumn("is_current", F.lit(True))
    ...
    try:
        ...
        (
            ...
            .whenMatchedUpdate(
                condition=F.expr(
                    "target.email <> source.email OR target.country <> source.country OR target.name <> source.name"
                ),
                set={
        ...
        src_with_sk = src_scd2.withColumn("customer_sk", F.row_number().over(window)).withColumn(
            "customer_sk", F.col("customer_sk") + F.lit(1000000)
        )  # Offset to avoid conflicts

        # Write initial table
        write_delta(src_with_sk, gold_path, mode="overwrite")
    ...
    logger.info(
        f"✅ dim_customer: {current_count:,} current records, {total_count:,} total (including history)"
    )
...
def main():
    ...
    try:
        # Build dim_customer
        build_dim_customer_scd2(spark, config)
