...
def build_dim_product(spark: SparkSession, config: dict[str, Any]) -> DataFrame:
    ...
    try:
        ...
        if "product_id" in orders.columns:
            ...
            return spark.createDataFrame(
                [], schema=spark.sql("SELECT 0 as product_sk, '' as product_id").schema
            )

    # Add surrogate key
    dim_product = (
        products.withColumn("product_sk", F.monotonically_increasing_id() + F.lit(1000000))
...
def build_gold_star_schema(
    ...
    dim_customer_current = dim_customer.filter(F.col("is_current"))
    ...
    fact_sales = (
        silver_orders.join(
            dim_customer_current.select("customer_sk", "customer_id"), on="customer_id", how="left"
        )
        .join(dim_product.select("product_sk", "product_id"), on="product_id", how="left")
        .join(
    ...
    write_delta(fact_sales, f"{gold_root}/fact_sales", mode="append", partitionBy=["order_date"])
    ...
    write_delta(
        fact_behavior, f"{gold_root}/fact_behavior", mode="append", partitionBy=["event_date"]
    )
