...
from pyspark.sql.functions import (
    ...
    row_number, rank, coalesce, lead, lag
)
...
def hash_record(df: DataFrame, key_cols: List[str], hash_col_name: str = "_hash") -> DataFrame:
    ...
    hash_expr = sha2(
        concat_ws("|", *[col(c) for c in key_cols]),
        256
    )
...
def update_customer_dimension_scd2(
    ...
    try:
        ...
        existing_with_hash = hash_record(
            existing_df,
            key_cols=[col for col in existing_df.columns 
                     if col not in [key_column, version_col, effective_from_col, effective_to_col, is_current_col, "_hash"]],
        ...
        new_records = comparison_df \
            .filter(col("target.customer_id").isNull()) \
        ...
        unchanged_count = comparison_df \
            .filter(
                col("source._source_hash") == col("target._target_hash")
            ) \
            .count()
        
        logger.info(f"📊 SCD2 Analysis:")
        ...
        # Add SCD2 metadata
        records_with_scd2 = records_to_insert \
            .withColumn(version_col, lit(1)) \
        ...
        update_condition = f"target.{key_column} = source.{key_column} AND target.{is_current_col} = true"
        
        (target.alias("target")
         .merge(
             changed_customer_ids.alias("source"),
             update_condition
         )
         .whenMatchedUpdate(
             set={
        ...
        records_with_versions = records_with_scd2.alias("new") \
            .join(
            ...
            .withColumn(
                version_col,
                when(col("max.max_version").isNotNull(), col("max.max_version") + 1)
                .otherwise(lit(1))
            ) \
            .drop("max_version")
        
        # Insert new records
        records_with_versions.write \
            .format("delta") \
            .mode("append") \
            .save(target_path)
