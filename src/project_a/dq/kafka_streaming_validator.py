...
class KafkaStreamingValidator:
    ...
    def _check_timestamp_monotonicity(self, df: DataFrame, timestamp_col: str) -> Dict[str, Any]:
        ...
        violations = df_with_prev.filter(
            (F.col(timestamp_col) < F.col("prev_timestamp")) &
    ...
    def _check_session_consistency(
        ...
        df_with_gaps = df.withColumn("time_diff", 
    ...
    def _check_cardinality(self, df: DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            "rows_per_customer": round(total_rows / customer_cardinality, 2) if customer_cardinality > 0 else 0,
            "rows_per_session": round(total_rows / session_cardinality, 2) if session_cardinality > 0 else 0
        }
    ...
    def _check_out_of_order(
        ...
        out_of_order = df_with_order.filter(
            (F.col(timestamp_col) < F.col("prev_ts")) &
            F.col("prev_ts").isNotNull()
        )
        
        out_of_order_count = out_of_order.count()
        sessions_affected = out_of_order.select(session_col).distinct().count() if out_of_order_count > 0 else 0
