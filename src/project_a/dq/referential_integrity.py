...
class ReferentialIntegrityChecker:
    ...
    def check_foreign_key(
        ...
        orphaned = fact_keys.join(dim_keys, fact_keys[fact_key] == dim_keys[dim_key], "left_anti")
    ...
    def check_orders_customers(
        self, orders_df: DataFrame, customers_df: DataFrame
    ) -> dict[str, Any]:
        """Check that all order.customer_id exist in customers."""
        return self.check_foreign_key(
            orders_df, customers_df, "customer_id", "customer_id", "orders → customers"
        )

    def check_orders_products(self, orders_df: DataFrame, products_df: DataFrame) -> dict[str, Any]:
        """Check that all order.product_id exist in products."""
        return self.check_foreign_key(
            orders_df, products_df, "product_id", "product_id", "orders → products"
        )

    def check_opportunities_accounts(
        self, opportunities_df: DataFrame, accounts_df: DataFrame
    ) -> dict[str, Any]:
        """Check that all opportunity.account_id exist in accounts."""
        return self.check_foreign_key(
            opportunities_df, accounts_df, "account_id", "account_id", "opportunities → accounts"
        )

    def check_behavior_customers(
        self, behavior_df: DataFrame, customers_df: DataFrame
    ) -> dict[str, Any]:
        """Check that all behavior.customer_id exist in customers."""
        return self.check_foreign_key(
            behavior_df, customers_df, "customer_id", "customer_id", "behavior → customers"
        )

    def check_kafka_customers(self, kafka_df: DataFrame, customers_df: DataFrame) -> dict[str, Any]:
    ...
    def check_duplicate_ids(self, df: DataFrame, id_column: str, table_name: str) -> dict[str, Any]:
        ...
        if duplicate_count > 0:
            ...
            result["duplicate_ids"] = [
                {"id": row[id_column], "count": row["count"]} for row in duplicate_sample
            ]
