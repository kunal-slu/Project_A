...
class MetadataCatalog:
    ...
    def update_dataset_stats(self, name: str, record_count: int = None, data_quality_score: float = None):
    ...
    def search_datasets(self, query: str = "", tags: List[str] = None) -> List[DatasetMetadata]:
        ...
        for file_path in self.datasets_path.glob("*.json"):
            with open(file_path, 'r') as f:
                ...
                matches_query = (
                    query.lower() in metadata.name.lower() or
                    query.lower() in metadata.description.lower() or
                    query.lower() in metadata.business_definition.lower()
                )
    ...
    def get_schema_compatibility(self, dataset_name: str, proposed_schema: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        changed_cols = {
            name for name in existing_cols.keys() & proposed_cols.keys()
...
def register_dataset(
    ...
    return catalog.register_dataset(
        name, location, format, df_schema, record_count, 
        owner, description, business_definition, tags, pii_fields
    )
