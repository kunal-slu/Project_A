...
class ArchiveManager:
    ...
    def __init__(self, archive_base_path: str = "data/archive", 
    ...
    def archive_dataset(self, dataset_path: str, policy_name: str, 
                       archive_reason: str = "retention_policy") -> str:
    ...
    def get_archive_info(self, archive_path: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'size_bytes': self._get_directory_size(archive_path_obj) if archive_path_obj.exists() else 0,
            'metadata': metadata
        }
...
class RetentionPolicyManager:
    ...
    def create_policy(self, name: str, dataset_path: str, retention_period_days: int,
                     archive_after_days: int, delete_after_days: int,
                     backup_enabled: bool = True, compression_enabled: bool = True,
                     notification_emails: List[str] = None) -> RetentionPolicy:
    ...
    def load_policy(self, name: str) -> Optional[RetentionPolicy]:
...
class LifecycleManager:
    ...
    def _find_applicable_policy(self, dataset_path: str) -> Optional[RetentionPolicy]:
        ...
        for policy_name in self.policy_manager.list_policies():
            ...
            if policy and str(Path(dataset_path).resolve()).startswith(str(Path(policy.dataset_path).resolve())):
                return policy
        return None
    ...
    def run_lifecycle_management(self, base_path: str = "data") -> List[Dict[str, Any]]:
        ...
        for item in base_path_obj.rglob('*'):
            ...
            elif item.is_dir() and any(f.suffix in ['.csv', '.parquet', '.json', '.delta'] for f in item.rglob('*')):
...
def create_retention_policy(name: str, dataset_path: str, retention_period_days: int,
                          archive_after_days: int, delete_after_days: int,
                          backup_enabled: bool = True, compression_enabled: bool = True,
                          notification_emails: List[str] = None) -> RetentionPolicy:
    ...
    return policy_mgr.create_policy(
        name, dataset_path, retention_period_days, archive_after_days,
        delete_after_days, backup_enabled, compression_enabled, notification_emails
    )


def archive_dataset(dataset_path: str, policy_name: str, 
                   archive_reason: str = "retention_policy") -> str:
    """Archive a dataset"""
    archive_mgr = get_archive_manager()
