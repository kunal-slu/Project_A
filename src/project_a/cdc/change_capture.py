...
class ChangeCaptureBuffer:
    ...
    def get_changes_since(
        self, table_name: str, since: datetime, change_types: list[ChangeType] = None
    ) -> list[ChangeRecord]:
                            ...
                            try:
                                ...
                                if record_time >= since:
                                    ...
                                    if (
                                        ...
                                        or ChangeType(record_data["change_type"]) in change_types
                                    ):
...
class IncrementalProcessor:
    ...
    def __init__(self, watermark_manager: WatermarkManager, change_buffer: ChangeCaptureBuffer):
    ...
    def detect_changes(
        ...
        for _idx, row in source_df.iterrows():
            ...
            change_record = ChangeRecord(
                change_id=hashlib.md5(
                    f"{table_name}_{row[primary_key]}_{change_time}".encode()
                ).hexdigest(),
    ...
    def get_incremental_data(
        self, table_name: str, since: datetime = None, limit: int = None
    ) -> pd.DataFrame:
        ...
        if records:
            ...
            return df.drop_duplicates(
                subset=[change.primary_key for change in changes[:1]], keep="last"
            )
...
def process_incremental_data(
    source_df: pd.DataFrame, table_name: str, primary_key: str, timestamp_column: str = None
) -> dict[str, Any]:
...
def get_incremental_data(
    table_name: str, since: datetime = None, limit: int = None
) -> pd.DataFrame:
...
def get_watermark(source: str, table: str) -> datetime | None:
    """Get the current watermark for a source and table"""
    watermark_mgr = get_watermark_manager()
