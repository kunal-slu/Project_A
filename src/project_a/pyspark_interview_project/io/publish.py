...
from project_a.utils.secrets import get_redshift_credentials, get_snowflake_credentials
...
def publish_to_snowflake(
    ...
    if mode == "merge" and keys:
        ...
        ).option("password", sf_options["sfPassword"]).option(
            "driver", "net.snowflake.client.jdbc.SnowflakeDriver"
        ).option("query", merge_sql).load().collect()
        ...
        spark.read.format("jdbc").option("url", jdbc_url).option(
            ...
            "driver", "net.snowflake.client.jdbc.SnowflakeDriver"
        ).option("query", f"DROP TABLE IF EXISTS {staging_table}").load().collect()

        logger.info(f"✅ MERGE completed for {table_name}")
    else:
        # Simple overwrite or append
        df.write.format("snowflake").options(**sf_options).option(
            "dbtable", table_name.split(".")[-1]
        ).mode(mode).save()
...
def publish_to_redshift(
    df: DataFrame, table_name: str, mode: str = "append", config: dict[str, Any] | None = None
) -> bool:
    """
    ...
    # Build JDBC URL
    jdbc_url = (
        f"jdbc:redshift://{creds.get('host')}:{creds.get('port', 5439)}/{creds.get('database')}"
    )

    df = df.withColumn("_load_ts", current_timestamp())

    # Write to Redshift
    df.write.format("jdbc").option("url", jdbc_url).option("user", creds.get("user")).option(
        "password", creds.get("password")
    ).option("driver", "com.amazon.redshift.jdbc.Driver").option("dbtable", table_name).mode(
        mode
    ).save()
