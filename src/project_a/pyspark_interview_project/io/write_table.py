    ...
    if fmt == "iceberg":
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Successfully wrote to Iceberg table using createOrReplace: {full_table_name}"
                )
        ...
        if (
            not table_name.startswith("s3://")
            and not table_name.startswith("s3a://")
...
def read_table(
    table_name: str, cfg: dict[str, Any] | None = None, spark: SparkSession | None = None
) -> DataFrame:
    """
