...
class LineageEmitter:
    ...
    def emit_job(
        ...
        try:
                        ...
                        "facets": {
                            "dataSource": {
                                ...
                                "uri": (
                                    ...
                                    else f"s3://{(self.config if hasattr(self, 'config') and self.config else {}).get('buckets', {}).get('lake', 'unknown-bucket')}/{input_name.replace('.', '/')}"
                                ),
                            }
                        },
                    }
                        ...
                        "facets": {
                            "dataSource": {
                                ...
                                "uri": (
                                    ...
                                    else f"s3://{(self.config if hasattr(self, 'config') and self.config else {}).get('buckets', {}).get('lake', 'unknown-bucket')}/{output_name.replace('.', '/')}"
                                ),
                            }
                        },
                    }
            ...
            # Send to backend (best-effort, no external 'requests' dependency)
            ...
            with urllib.request.urlopen(req, timeout=10) as resp:
                # Read response to ensure the request is executed; ignore body
                _ = resp.read()
