...
def put_metric(
    name: str, value: float, unit: str = "Count", dims: list[dict[str, str]] | None = None
):
    """
...
def emit_duration(name: str, duration_seconds: float, dims: list[dict[str, str]] | None = None):
    """Emit duration metric to CloudWatch."""
    put_metric(name, duration_seconds, unit="Seconds", dims=dims)


def emit_count(name: str, count: int, dims: list[dict[str, str]] | None = None):
    """Emit count metric to CloudWatch."""
    put_metric(name, float(count), unit="Count", dims=dims)


def emit_size(name: str, size_bytes: int, dims: list[dict[str, str]] | None = None):
...
def emit_dq_metrics(results: dict, suite_name: str, layer: str):
    ...
    emit_count("dq_validations_total", len(results.get("expectations", [])), dims=dims)

    emit_count("dq_critical_failures", results.get("critical_failures", 0), dims=dims)

    emit_count("dq_warning_failures", results.get("warning_failures", 0), dims=dims)
