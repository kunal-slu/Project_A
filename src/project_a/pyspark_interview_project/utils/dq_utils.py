...
def run_dq_suite(
    spark: SparkSession, df: DataFrame, suite_name: str, expectations: list[dict[str, Any]]
) -> dict[str, Any]:
    """
    ...
    for expectation in expectations:
        ...
        try:
            ...
            result = {"expectation_type": exp_type, "passed": passed, "kwargs": kwargs}
            ...
            results["expectations"].append(
                {"expectation_type": exp_type, "passed": False, "error": str(e)}
            )

    logger.info(
        f"DQ suite {suite_name}: {results['passed']}/{results['total_expectations']} passed"
    )
    return results
