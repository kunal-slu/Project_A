...
from pyspark.sql.functions import col, concat, lit, regexp_replace, sha2, when
...
def mask_email(df: DataFrame, email_col: str, output_col: str | None = None) -> DataFrame:
    """
...
def mask_phone(df: DataFrame, phone_col: str, output_col: str | None = None) -> DataFrame:
    """
    ...
    masked_expr = regexp_replace(col(phone_col), r"^(\+?\d{1,2})?(\d{1,3})?(\d{4})$", r"\1***\3")

    # If pattern doesn't match, just mask all but last 4
    masked_expr = when(
        col(phone_col).rlike(r"^\d+$"), regexp_replace(col(phone_col), r"^(.+?)(\d{4})$", r"***\2")
...
def hash_value(
    df: DataFrame, col_name: str, output_col: str | None = None, salt_from_env: bool = True
) -> DataFrame:
    """
    ...
    hashed_expr = sha2(when(col(col_name).isNotNull(), concat(lit(salt), col(col_name))), 256)

    return df.withColumn(output_col, hashed_expr)


def mask_name(df: DataFrame, name_col: str, output_col: str | None = None) -> DataFrame:
    """
    ...
    masked_expr = regexp_replace(col(name_col), r"(\w)\w*", r"\1***")

    return df.withColumn(output_col, masked_expr)


def apply_pii_masking(df: DataFrame, pii_config: dict, layer: str = "gold") -> DataFrame:
    """
