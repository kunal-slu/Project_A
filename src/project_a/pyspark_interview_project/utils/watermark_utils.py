...
def get_watermark(
    source_name: str, config: dict = None, spark: SparkSession = None
) -> datetime | None:
    """
    ...
    if config and spark:
        watermark_path = config.get("data_lake", {}).get(
            "watermark_prefix", "s3://bucket/meta/watermarks"
        )
        if watermark_path.startswith("s3://") or watermark_path.startswith("s3a://"):
            try:
                ...
                if not watermark_df.isEmpty():
                    watermark_value = watermark_df.select(spark_max("watermark_value")).collect()[
                        0
                    ][0]
                    if watermark_value:
...
def upsert_watermark(
    source_name: str, value: datetime, config: dict = None, spark: SparkSession = None
) -> None:
    """
    ...
    watermark_df = spark.createDataFrame(
        [(source_name, value, datetime.now(timezone.utc))], schema=schema
    )
    ...
    if (
        watermark_path
        and (watermark_path.startswith("s3://") or watermark_path.startswith("s3a://"))
        and spark
    ):
        ...
        try:
            ...
            if DeltaTable.isDeltaTable(spark, full_path):
                ...
                delta_table.alias("target").merge(
                    watermark_df.alias("source"), "target.source_name = source.source_name"
...
def get_latest_timestamp_from_df(
    df: DataFrame, timestamp_col: str = "_ingestion_ts"
) -> datetime | None:
    """
