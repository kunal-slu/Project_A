...
def align_to_schema(
    ...
    """
    logger.info(f"Aligning DataFrame to schema contract...")
    ...
    if extra_cols and extra_cols_to_metadata:
        ...
        df = df.withColumn(
            "metadata_extra",
            F.to_json(F.struct(*extra_struct_cols))
        )
    ...
    logger.info(f"✅ Schema alignment complete")
...
def validate_and_quarantine(
    ...
    error_lane_path: Optional[str] = None
    ...
    if quarantine_count > 0 and error_lane_path:
        ...
        logger.info(f"✅ Quarantined rows written to error lane")
    ...
    logger.info(f"✅ Validation complete: {validation_results['clean_rows']}/{validation_results['total_rows']} rows passed")
