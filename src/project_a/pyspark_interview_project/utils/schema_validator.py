...
class SchemaValidator:
    ...
    def validate_data_quality(self, df: DataFrame) -> dict[str, Any]:
        ...
        validation_rules.get("format_checks", {})
                ...
                if "min" in range_def:
                    ...
                    if below_min > 0:
                        results["issues"].append(
                            f"{field}: {below_min} values below minimum {range_def['min']}"
                        )
                if "max" in range_def:
                    above_max = df.filter(col(field) > range_def["max"]).count()
                    if above_max > 0:
                        results["issues"].append(
                            f"{field}: {above_max} values above maximum {range_def['max']}"
                        )
    ...
    def validate_and_prepare(
        ...
        if "_source_system" not in aligned_df.columns:
            aligned_df = aligned_df.withColumn(
                "_source_system", lit(self.schema_def.get("source_system", "unknown"))
            )
...
def validate_schema(
    ...
    if missing_columns:
        ...
        for col_name in missing_columns:
            col_def = next(
                (c for c in expected_schema.get("columns", []) if c["name"] == col_name), None
            )
            if col_def:
...
def log_schema_drift(
    ...
    if config and spark:
        drift_path = config.get("data_lake", {}).get(
            "drift_prefix", "s3://bucket/meta/schema_drifts"
        )
