    ...
    try:
        ...
        if date_col and orders_df.count() > 0:
            min_max = orders_df.agg(
                F.min(F.col(date_col)).alias("min_date"), F.max(F.col(date_col)).alias("max_date")
            ).collect()[0]
        ...
        logger.warning(
            f"Could not determine date range from orders: {e}, using default 2-year range"
        )
        from datetime import date, timedelta
...
def build_dim_customer(
    customers_silver: DataFrame, spark: SparkSession | None = None
) -> DataFrame:
    """
...
def build_dim_product(
    products_silver: DataFrame, spark: SparkSession | None = None
) -> DataFrame:
    """
...
def build_fact_orders(
    ...
    orders = orders_silver.withColumn(
        "sales_amount", F.coalesce(F.col("amount_usd"), F.col("amount_orig"), F.lit(0.0))
    )
    ...
    dim_customer_with_sk = dim_customer.withColumn(
        "customer_sk", F.monotonically_increasing_id().cast("long")
    )

    dim_product_with_sk = dim_product.withColumn(
        "product_sk", F.monotonically_increasing_id().cast("long")
    )
    ...
    fact = (
        orders.join(
            dim_customer_with_sk.select("customer_id", "customer_sk"), "customer_id", "left"
        )
        .join(dim_product_with_sk.select("product_id", "product_sk"), "product_id", "left")
        .join(dim_date.select("date", "date_sk"), F.col("order_date") == F.col("date"), "left")
...
def build_customer_360(
    ...
    # Join all sources
    customer_360 = dim_customer.join(order_agg, "customer_id", "left").join(
...
def build_product_performance(
    dim_product: DataFrame, orders_silver: DataFrame, spark: SparkSession | None = None
) -> DataFrame:
    """
