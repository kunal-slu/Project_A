...
def load_with_fallback(
    ...
    try:
        ...
        if schema:
            ...
            raise ValueError(
                f"Could not load data from {path} and no schema provided for empty DataFrame"
            )
...
def load_crm_bronze_data(
    ...
    try:
        ...
        base_path = crm_cfg.get(
            "base_path", config.get("paths", {}).get("bronze_root", "") + "/crm"
        )
...
def load_snowflake_bronze_data(
    ...
    try:
        ...
        base_path = snowflake_cfg.get(
            "base_path", config.get("paths", {}).get("bronze_root", "") + "/snowflake"
        )
...
def load_redshift_behavior_bronze_data(spark: SparkSession, config: dict[str, Any]) -> DataFrame:
    """
    ...
    try:
        ...
        base_path = redshift_cfg.get(
            "base_path", config.get("paths", {}).get("bronze_root", "") + "/redshift"
        )
        files = redshift_cfg.get("files", {})
        behavior_path = (
            f"{base_path}/{files.get('behavior', 'redshift_customer_behavior_50000.csv')}"
        )
...
def load_kafka_bronze_data(spark: SparkSession, config: dict[str, Any]) -> DataFrame:
    """
    ...
    try:
        ...
        base_path = kafka_cfg.get(
            "base_path", config.get("paths", {}).get("bronze_root", "") + "/kafka"
        )
