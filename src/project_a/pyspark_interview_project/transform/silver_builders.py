...
def build_customers_silver(
    ...
    customers_base = df_customers.select(
        ...
        F.coalesce(F.col("lifetime_value"), F.lit(0.0)).alias("annual_revenue")  # Use lifetime_value as proxy
    ...
    customers = customers_base.join(last_contact, "customer_id", "left") \
