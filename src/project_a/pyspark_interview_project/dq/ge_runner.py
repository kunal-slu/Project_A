...
class GERunner:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.dq_results_path = config.get('paths', {}).get('dq_results_root') or config.get('data_lake', {}).get('dq_results_path', 's3://my-etl-lake-demo/_dq_results')
    ...
    def run_suite(
        ...
        if results["critical_failures"] > 0:
            error_msg = f"DQ critical failures: {results['critical_failures']} violations in {suite_name}"
            logger.error(error_msg)
            raise ValueError(error_msg)
        
        logger.info(f"✅ DQ suite passed: {suite_name} ({len(results['expectations'])} expectations)")
