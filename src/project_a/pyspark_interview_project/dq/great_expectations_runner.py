...
class GreatExpectationsRunner:
    ...
    def __init__(self, context_root: Path | None = None):
        """
    ...
    def init_context(self) -> None:
        ...
        try:
            ...
            if context_path.exists():
                ...
                logger.warning(
                    f"GE context not found at {context_path}. Run 'great_expectations init' first."
                )
                self.context = None
    ...
    def run_checkpoint(
        ...
        if not self.context:
            ...
            return {"success": True, "skipped": True, "message": "GE not configured"}
        ...
        try:
            ...
            if not success and fail_on_error:
                ...
                for failure in summary["failures"]:
                    logger.error(f"  - {failure['expectation_type']}: {failure['kwargs']}")

                raise RuntimeError(error_msg)

            return {"success": success, "summary": summary, "data_docs_url": data_docs_url}
    ...
    def _build_summary(self, result: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            for run_id, run_result in run_results.items():
                ...
                summary["total_expectations"] += statistics.get("evaluated_expectations", 0)
                summary["successful_expectations"] += statistics.get("successful_expectations", 0)
                ...
                for exp_result in results:
                    if not exp_result.get("success", True):
                        ...
                        summary["failures"].append(
                            {
                                "expectation_type": exp_result.get("expectation_config", {}).get(
                                    "expectation_type"
                                ),
    ...
    def _get_data_docs_url(self, result: dict[str, Any]) -> str | None:
        """
    ...
    def validate_dataframe(
        self, df: "pyspark.sql.DataFrame", expectation_suite_name: str, fail_on_error: bool = True
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            # Fail fast if needed
            if not success and fail_on_error:
                failed_count = statistics.get("evaluated_expectations", 0) - statistics.get(
                    "successful_expectations", 0
                )
                error_msg = (
...
def run_dq_checkpoint(
    checkpoint_name: str, context_root: Path | None = None, fail_on_error: bool = True
) -> dict[str, Any]:
    """
