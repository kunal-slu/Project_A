...
class StandardETLPipeline:
    ...
    def _generate_customer_data(self):
        ...
        created_dates = [
            datetime.now() - timedelta(days=np.random.randint(1, 365)) for _ in range(n_customers)
        ]
        segments = np.random.choice(
            ["Basic", "Standard", "Premium"], n_customers, p=[0.4, 0.4, 0.2]
        )
    ...
    def _generate_order_data(self):
        ...
        order_dates = [
            datetime.now() - timedelta(days=np.random.randint(1, 30)) for _ in range(n_orders)
        ]
        amounts = np.random.uniform(10, 1000, n_orders)
        statuses = np.random.choice(
            ["completed", "pending", "cancelled"], n_orders, p=[0.8, 0.15, 0.05]
        )
    ...
    def _generate_monthly_revenue(self):
        ...
        return pd.DataFrame({"month": months, "total_revenue": total_revenues})
    ...
    def process_bronze_layer(self):
        ...
        for table_dir in os.listdir(bronze_path):
            ...
            if os.path.isdir(table_path):
                ...
                table_info = delta_lake.get_table_info()
                logger.info(
                    f"    Versions: {table_info['versions']}, Records: {table_info['parquet_files']}"
                )
...
def main():
    ...
    try:
        ...
        config_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "..", "config", "local.yaml"
        )
