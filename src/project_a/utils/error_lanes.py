...
class ErrorLaneHandler:
    ...
    def __init__(self, spark: SparkSession, config: Dict[str, Any]):
        ...
        if lake_bucket:
            self.lake_root = f"s3://{lake_bucket}"
        else:
            self.lake_root = config.get("paths", {}).get("bronze_root", "").replace("/bronze", "")
        self.errors_prefix = f"{self.lake_root.rstrip('/')}/_errors"

    def get_error_lane_path(
        self, layer: str, table: str, run_id: str, partition_date: str = None
    ) -> str:
        """
    ...
    def quarantine_bad_rows(
        ...
        if validation_errors.count() > 0:
            ...
            # Add error metadata
            quarantined_df = quarantined_df.withColumn(
                "_error_timestamp", F.current_timestamp()
        ...
        logger.info(
            f"✅ Quarantine complete: {stats['clean_rows']}/{stats['total_rows']} rows passed"
        )
    ...
    def quarantine_schema_violations(
        ...
        if quarantine_count > 0:
            ...
            quarantined_df.withColumn("_error_timestamp", F.current_timestamp()).withColumn(
                "_error_type", F.lit("schema_violation")
            ).write.format("json").mode("append").save(error_path)
