...
class BuildManager:
    ...
    def build_artifact(
        self, source_path: str, artifact_name: str, build_script: str = "./build.sh"
    ) -> str:
        ...
        try:
            ...
            final_artifact = (
                self.builds_path
                / f"{artifact_name}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.whl"
            )
...
class DeploymentManager:
    ...
    def deploy_to_environment(
        self, artifact_path: str, environment: str, deployment_script: str = "./deploy.sh"
    ) -> dict[str, Any]:
...
class PipelineManager:
    ...
    def __init__(
        self, pipeline_config_path: str = "config/pipeline.yaml", artifacts_path: str = "artifacts"
    ):
    ...
    def _load_pipeline_config(self) -> dict[str, Any]:
        ...
        if self.pipeline_config_path.exists():
            ...
            return {
                "stages": {
                    "build": {"command": "python setup.py bdist_wheel", "timeout": 300},
                    "test": {"command": "python -m pytest tests/", "timeout": 600},
                    "integration_test": {
                        "command": "python -m pytest tests/integration/",
                        "timeout": 1200,
                    },
                    "staging": {"command": "./deploy_staging.sh", "timeout": 900},
                    "production": {
    ...
    def _execute_build_stage(
        self, job: PipelineJob, stage_config: dict[str, Any]
    ) -> dict[str, Any]:
        """Execute build stage"""
        try:
    ...
    def _execute_integration_test_stage(
        self, job: PipelineJob, stage_config: dict[str, Any]
    ) -> dict[str, Any]:
        """Execute integration test stage"""
        # Similar to test stage but for integration tests
        test_result = self.build_manager.run_unit_tests(
            job.command or "python -m pytest tests/integration/"
        )
        return test_result

    def _execute_deploy_stage(
        self, job: PipelineJob, environment: str, stage_config: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        for _prev_job in job.dependencies or []:
...
class PipelineOrchestrator:
    ...
    def schedule_pipeline(
        self, pipeline_name: str, schedule: str, trigger_source: str = "scheduler"
    ) -> dict[str, Any]:
...
def get_pipeline_manager() -> PipelineManager:
    ...
    if _pipeline_manager is None:
        ...
        pipeline_config_path = config.get("paths", {}).get(
            "pipeline_config_root", "config/pipeline.yaml"
        )
...
def schedule_pipeline(
    pipeline_name: str, schedule: str, trigger_source: str = "scheduler"
) -> dict[str, Any]:
    """Schedule a pipeline to run on a schedule"""
    orchestrator = get_pipeline_orchestrator()
...
def get_pipeline_status(run_id: str) -> PipelineRun | None:
    """Get status of a pipeline run"""
    pipeline_mgr = get_pipeline_manager()
