...
class BackupManager:
    ...
    def __init__(self, backup_base_path: str = "backups", 
                 config_path: str = "config/backups"):
        ...
        for source_path in plan.source_paths:
            ...
            if source_path_obj.is_file():
                ...
                if plan.compression_enabled:
                    ...
                    compressed_path = backup_path / f"{source_path_obj.name}.zip"
                    shutil.make_archive(str(compressed_path.with_suffix('')), 'zip', source_path_obj)
                    backed_up_paths.append(str(compressed_path))
                else:
...
class RecoveryManager:
    ...
    def _find_latest_backup(self, backup_source: str) -> Optional[str]:
...
class DRPlanManager:
    ...
    def create_backup_plan(self, name: str, schedule: str, source_paths: List[str],
                          destination_path: str, retention_days: int,
                          backup_type: str = "full", encryption_enabled: bool = True,
                          compression_enabled: bool = True,
                          notification_emails: List[str] = None) -> BackupPlan:
    ...
    def create_recovery_plan(self, name: str, backup_source: str,
                            recovery_targets: List[str],
                            recovery_point_objective: int = 24,
                            recovery_time_objective: int = 4,
                            priority: str = "medium",
                            notification_emails: List[str] = None) -> RecoveryPlan:
    ...
    def get_backup_plan(self, name: str) -> Optional[BackupPlan]:
    ...
    def get_recovery_plan(self, name: str) -> Optional[RecoveryPlan]:
...
def create_backup_plan(name: str, schedule: str, source_paths: List[str],
                      destination_path: str, retention_days: int,
                      backup_type: str = "full", encryption_enabled: bool = True,
                      compression_enabled: bool = True,
                      notification_emails: List[str] = None) -> BackupPlan:
    ...
    return dr_mgr.create_backup_plan(
        name, schedule, source_paths, destination_path, retention_days,
        backup_type, encryption_enabled, compression_enabled, notification_emails
    )


def create_recovery_plan(name: str, backup_source: str,
                        recovery_targets: List[str],
                        recovery_point_objective: int = 24,
                        recovery_time_objective: int = 4,
                        priority: str = "medium",
    ...
    return dr_mgr.create_recovery_plan(
        name, backup_source, recovery_targets,
        recovery_point_objective, recovery_time_objective,
        priority, notification_emails
    )
