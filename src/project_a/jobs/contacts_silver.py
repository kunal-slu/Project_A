...
from project_a.delta_utils import merge_publish, spark_session, write_staging
...
def transform_contacts(bronze_df):
    ...
    return (
        bronze_df.withColumn("email", F.lower(F.trim(F.col("email"))))
...
def run(bronze_path: str, staging_path: str, publish_path: str, config: dict = None) -> dict:
    """
    ...
    try:
        ...
        bronze_df = spark.read.schema(CONTACTS_SCHEMA).format("delta").load(bronze_path)
        ...
        duplicate_contacts = (
            staging_df.groupBy("contact_id").count().filter(F.col("count") > 1).count()
        )
