...
def publish_gold_idempotent(source_df, gold_path: str, key_columns: list, 
                           merge_condition: str = None, spark_session=None):
    """
...
def publish_customer_analytics(source_df, gold_path: str = "data/lakehouse_delta/gold/customer_analytics"):
...
def publish_order_analytics(source_df, gold_path: str = "data/lakehouse_delta/gold/order_analytics"):
    """Publish order analytics with idempotent MERGE"""
    return publish_gold_idempotent(
        source_df=source_df,
        gold_path=gold_path,
        key_columns=["amount_category"]
    )

def publish_monthly_revenue(source_df, gold_path: str = "data/lakehouse_delta/gold/monthly_revenue"):
