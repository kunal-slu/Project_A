...
def extract_salesforce_leads(
    spark: SparkSession,
    config: Dict[str, Any],
    table_name: str
) -> DataFrame:
    """
    Extracts Salesforce leads data.
    ...
    Returns:
        DataFrame: Salesforce leads data
    """
    logger.info(f"Extracting Salesforce leads for table: {table_name}")

    # For local dev, use sample data if available
    if config.get('environment') == 'local':
        sample_path = "aws/data/salesforce/salesforce_leads.csv"
    ...
    logger.info(f"Extracted {df.count()} records for Salesforce leads.")
    return df