"""
Returns Pipeline DAG with OpenLineage support.
Processes returns data through bronze, silver, and gold layers.
"""
...
dag = DAG(
    ...
    description='Returns data pipeline with bronze, silver, and gold processing',
    schedule_interval='@daily',
    catchup=False,
    max_active_runs=1,
    tags=['data-pipeline', 'returns', 'bronze', 'silver', 'gold'],
)
...
emr_job_config = {
    ...
    "jobDriver": {
        "sparkSubmit": {
            "entryPoint": "s3://{}/jobs/bronze_to_silver.py".format(CODE_BUCKET),
            "entryPointArguments": [
                "--config", "s3://{}/config/application-aws.yaml".format(CODE_BUCKET)
            ],
...
send_notification = BashOperator(
    ...
    bash_command="""
    echo "Returns pipeline completed successfully!"
