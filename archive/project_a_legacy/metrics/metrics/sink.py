...
class MetricsSink:
    ...
    def incr(self, name: str, value: float, dims: dict[str, str] | None = None):
        pass

    def gauge(self, name: str, value: float, dims: dict[str, str] | None = None):
        pass

    def timing(self, name: str, duration_ms: float, dims: dict[str, str] | None = None):
        pass

    def pipeline_metric(
        self, stage: str, metric_type: str, value: float, dims: dict[str, str] | None = None
    ):
    ...
    def data_quality_metric(
        self, table: str, metric_type: str, value: float, dims: dict[str, str] | None = None
    ):
...
class StdoutSink(MetricsSink):
    ...
    def pipeline_metric(self, stage, metric_type, value, dims=None):
        ...
        metric_data = {
            ...
            "type": "gauge",
            "value": value,
            "dims": {"stage": stage, "metric_type": metric_type, **(dims or {})},
            "run_id": self.run_id,
            "timestamp": int(time.time() * 1000),
        }
        print(f"METRIC: {metric_data}")
    ...
    def data_quality_metric(self, table, metric_type, value, dims=None):
        ...
        metric_data = {
            ...
            "type": "gauge",
            "value": value,
            "dims": {"table": table, "metric_type": metric_type, **(dims or {})},
            "run_id": self.run_id,
            "timestamp": int(time.time() * 1000),
        }
        print(f"METRIC: {metric_data}")
