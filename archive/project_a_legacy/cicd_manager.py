...
class CICDManager:
    ...
    def create_deployment(
        ...
        try:
            ...
            if config_file is None:
                config_file = env_config.get(
                    "default_config_file", f"configs/{environment.value}.yaml"
                )
            ...
            logger.info(
                f"Created deployment configuration for {environment.value} version {version}"
            )
    ...
    def deploy_to_environment(self, deployment_config: DeploymentConfig) -> DeploymentResult:
        ...
        try:
            ...
            logger.info(
                f"Starting deployment {deployment_id} to {deployment_config.environment.value}"
            )
            ...
            if not self._validate_deployment(deployment_config, deployment_result):
                ...
                deployment_result.duration_minutes = (
    ...
    def _validate_deployment(
        self, deployment_config: DeploymentConfig, deployment_result: DeploymentResult
    ) -> bool:
    ...
    def _run_security_tests(self, deployment_result: DeploymentResult) -> bool:
        ...
        try:
            ...
            if test_result.returncode != 0:
                deployment_result.warnings.append(
                    f"Security tests found issues: {test_result.stdout}"
                )
    ...
    def _execute_deployment(
        ...
        try:
            self.environment_configs[deployment_config.environment]
            deployment_result.logs.append(
                f"Executing {deployment_config.deployment_type.value} deployment..."
            )

            # Load configuration
            with open(deployment_config.config_file) as f:
                if deployment_config.config_file.endswith(
                    ".yaml"
                ) or deployment_config.config_file.endswith(".yml"):
                    config_data = yaml.safe_load(f)
                else:
                    config_data = json.load(f)

            # Apply environment variables
            config_data = self._apply_environment_variables(
                config_data, deployment_config.variables
            )
    ...
    def _execute_blue_green_deployment(
        self, config_data: dict[str, Any], deployment_result: DeploymentResult
    ) -> bool:
    ...
    def _execute_rolling_deployment(
        ...
        try:
            ...
            for i, target in enumerate(targets):
                deployment_result.logs.append(
                    f"Deploying to target {i + 1}/{len(targets)}: {target}"
                )
                time.sleep(2)  # Simulate deployment time
    ...
    def _execute_immediate_deployment(
        self, config_data: dict[str, Any], deployment_result: DeploymentResult
    ) -> bool:
    ...
    def _run_health_checks_rolling(
        self, config_data: dict[str, Any], deployment_result: DeploymentResult
    ) -> bool:
    ...
    def _run_health_checks_immediate(
        self, config_data: dict[str, Any], deployment_result: DeploymentResult
    ) -> bool:
    ...
    def _run_canary_health_check(
        self, traffic_percentage: int, deployment_result: DeploymentResult
    ) -> bool:
    ...
    def get_deployment_history(
        self, environment: Environment = None, status: DeploymentStatus = None
    ...
    def export_deployment_report(
        self, output_path: str = "data/cicd/deployment_report.json"
    ) -> bool:
        ...
        try:
            ...
            for deploy_type in DeploymentType:
                count = len(
                    [d for d in self.deployment_history if d.deployment_type == deploy_type]
                )
                report_data["deployments_by_type"][deploy_type.value] = count

            # Get recent deployments (last 10)
            recent_deployments = sorted(
                self.deployment_history, key=lambda x: x.start_time, reverse=True
            )[:10]
...
def run_deployment_pipeline(
    ...
    try:
        ...
        deployment_config = cicd_manager.create_deployment(
            environment=environment, version=version, deployment_type=deployment_type
        )
