...
def write_stream(df: DataFrame, cfg: dict, table_path_logical: str, checkpoint_logical: str):
    ...
    query = (
        df_ev.writeStream.format("delta")
...
def create_dlq_handler(cfg: dict, checkpoint_logical: str):
    ...
    def handle_dlq(malformed_df: DataFrame, error_reason: str):
        ...
        if malformed_df.count() > 0:
            # Add error metadata
            dlq_df = malformed_df.withColumn("dlq_timestamp", F.current_timestamp()).withColumn(
                "error_reason", F.lit(error_reason)
            )
            ...
            logger.warning(
                f"Writing {malformed_df.count()} malformed records to DLQ: {error_reason}"
            )
            return dlq_query
...
def validate_streaming_config(cfg: dict):
    ...
    if not isinstance(watermark, str) or not any(
        unit in watermark for unit in ["hour", "minute", "second", "day"]
    ):
        raise ValueError(
            f"Invalid watermark format: {watermark}. Expected format like '1 hour', '30 minutes', etc."
        )
