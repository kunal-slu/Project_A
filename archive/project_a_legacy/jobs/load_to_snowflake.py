...
def load_delta_to_snowflake(
    ...
    try:
        ...
        if merge_keys:
            ...
            spark.read.format("jdbc").option("url", sf_url).option(
            ...
            df.write.format("snowflake").options(**sf_options).option(
                "dbtable", snowflake_table.split(".")[-1]
            ).mode("overwrite").save()
...
def _build_merge_sql(
    target_table: str, staging_table: str, merge_keys: list[str], columns: list[str]
) -> str:
    ...
    # Build ON clause
    on_clause = " AND ".join([f"{target_alias}.{key} = {source_alias}.{key}" for key in merge_keys])
...
def load_gold_tables_to_snowflake(config: dict[str, Any], tables: list[str] = None) -> bool:
    ...
    """
    if tables is None:
        tables = ["customer_360", "orders_metrics", "sales_fact"]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--env", choices=["dev", "prod", "local"], default="prod", help="Environment"
    )
