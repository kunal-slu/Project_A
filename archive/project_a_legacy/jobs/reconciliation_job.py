...
def compute_table_hash(df: DataFrame, key_columns: list[str] = None) -> str:
    ...
    hash_expr = sha2(concat_ws("|", *[col(c) for c in key_columns]), 256)
...
def reconcile_snowflake_to_s3(
    ...
    return reconcile_tables(
        source_df, target_df, key_columns, source_name="Snowflake", target_name="S3"
    )
...
def reconcile_redshift_to_s3(
    ...
    return reconcile_tables(
        source_df, target_df, key_columns, source_name="Redshift", target_name="S3"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source", choices=["snowflake", "redshift"], required=True, help="Source system"
    )
    ...
    parser.add_argument(
        "--key-columns", nargs="+", required=True, help="Key columns for reconciliation"
    )
