...
from project_a.utils.watermark import load_watermark, save_watermark
...
def incremental_customer_dim_upsert(
    spark: SparkSession, config: dict[str, Any], watermark_checkpoint_path: str = "data/checkpoints"
) -> None:
    """
    ...
    customer_updates = deduplicate_by_latest(
        customer_updates, key_col="customer_id", ts_col="last_modified_date"
    )
    ...
    if table_exists:
        ...
        silver_table.alias("target").merge(
            customer_updates.alias("source"), merge_condition
        ).whenMatchedUpdate(
        ...
        customer_updates.withColumn("created_at", current_timestamp()).withColumn(
            "updated_at", current_timestamp()
        ).write.format("delta").mode("overwrite").option("mergeSchema", "true").partitionBy(
            "_source_system"
...
def main():
    ...
    parser.add_argument(
        "--checkpoint-path", default="data/checkpoints", help="Watermark checkpoint path"
    )
