...
def write_bronze_idempotent(
    ...
    # Add run metadata
    df_with_metadata = df.withColumn("_run_id", lit(run_id)).withColumn(
    ...
    staging_df.write.format("delta").mode("append").option("mergeSchema", "true").partitionBy(
        *partition_cols
    ).save(final_path)
...
def write_silver_idempotent(
    df: DataFrame, output_path: str, table_name: str, key_columns: list, run_id: str | None = None
) -> str:
    """
    ...
    try:
        ...
        df_with_metadata.write.format("delta").mode("overwrite").option(
            "overwriteSchema", "true"
        ).save(output_path)

        logger.info("✅ New Silver table created")
