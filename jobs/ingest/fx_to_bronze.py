...
class FxToBronzeJob(BaseJob):
    ...
    def run(self, ctx) -> dict[str, Any]:
        ...
        try:
            ...
            source_path = (
                self.config.get("sources", {}).get("fx", {}).get("raw_path", "data/samples/fx")
            )
            ...
            fx_rates_df = spark.read.schema(FX_RATES_SCHEMA).json(
                f"{source_path}/fx_rates_historical.json"
            )
            ...
            check_date_realism(fx_rates_df, "bronze.fx.rates", self.config)

            # Log lineage
            self.log_lineage(
                source="fx_rates",
                target="bronze.fx",
                records_processed={"fx_rates": fx_rates_df.count()},
            )
