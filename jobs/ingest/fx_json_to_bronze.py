...
def extract_fx_json_to_bronze(
    ...
    try:
        ...
        error_path = error_handler.get_error_lane_path("bronze", "fx", run_id)
        ...
        df_clean = df_clean.withColumn("_ingest_ts", F.current_timestamp()) \
        ...
        logger.info(f"✅ Cleaned data: {final_count:,} records (routed {bad_count:,} to error lane)")
        
        # Emit metrics
        duration_ms = (time.time() - start_time) * 1000
        emit_rowcount("records_processed_total", final_count, {
            "job": "bronze_fx_json",
            "source": "fx",
            "format": "json"
        }, config)
...
def main():
    ...
    try:
        ...
        df_partitioned.write \
            ...
            .partitionBy("trade_date", "_run_date") \
            .save(bronze_fx_path)
        ...
        logger.info(f"🎉 FX JSON to Bronze job completed: {rows_out:,} records in {duration_ms:.0f}ms")
        return 0
