...
def get_snowflake_options(config: dict[str, Any]) -> dict[str, str]:
    ...
    if not password and "password_secret_name" in sf_cfg:
        try:
            ...
            sm = boto3.client(
                "secretsmanager", region_name=config.get("aws", {}).get("region", "us-east-1")
            )
...
def load_gold_table(spark: SparkSession, config: dict[str, Any], table_name: str) -> DataFrame:
    """
...
def merge_into_snowflake(
    sf_options: dict[str, str], target_table: str, staging_table: str, merge_keys: list = None
) -> None:
    """
    ...
    try:
        ...
        if merge_keys is None:
            ...
            if "FACT" in target_table.upper():
                merge_keys = (
                    ["ORDER_ID", "ORDER_DATE"]
                    if "ORDER" in target_table.upper()
                    else ["ID", "DATE"]
                )
            elif "DIM" in target_table.upper():
                merge_keys = (
                    ["CUSTOMER_ID"] if "CUSTOMER" in target_table.upper() else ["PRODUCT_ID"]
                )
            else:
                merge_keys = ["ID"]
...
def publish_to_snowflake(
    df: DataFrame, sf_options: dict[str, str], target_table: str, mode: str = "merge"
) -> None:
    """
    ...
    try:
        if mode == "merge":
            ...
            logger.info(
                f"✅ Successfully published {record_count:,} records to {target_table} via MERGE"
            )
...
def main():
    ...
    parser.add_argument(
        "--table",
        default="orders",
        help="Gold table to publish (orders, customer, product, customer_360)",
    )
    parser.add_argument(
        "--mode",
        default="merge",
        choices=["merge", "overwrite", "append"],
        help="Write mode (merge=staging+MERGE, overwrite=direct overwrite, append=direct append)",
    )
    ...
    logger.info(
        f"🚀 Starting Gold to Snowflake publish job (trace_id={trace_id}, table={args.table})"
    )
